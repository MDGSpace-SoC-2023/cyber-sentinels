...
function changestart() {
  ...
  for (var i = 0; i < dates.length; i++) {
    if (new Date(startDate.value) > new Date(dates[i].innerHTML.trim())) {
      if (
        !dates[i].parentElement.parentElement.classList.contains("notvisible")
      ) {
...
function Filterntfctns(c) {
  var buttons = document.getElementsByClassName("Act");
  for (var i = 0; i < buttons.length; i++) {
    buttons[i].style.color = "#fff";
  }
  var s = c + "Act";
  var button = document.getElementById(s);
  button.style.color = "#ffe66b";
  if (c === "All") {
    c = "ntfcation";
  }
...
function FilterSevere(c) {
  var notifications = document.getElementsByClassName("ntfcation");
  var buttons = document.getElementsByClassName("Sev");
  for (var i = 0; i < buttons.length; i++) {
    buttons[i].style.color = "#fff";
  }
  var s = c + "Sev";
  ...
  if (c === "All") {
    ...
    for (var i = 0; i < Severentfications.length; i++) {
      if (
        Severentfications[i].parentElement.parentElement.classList.contains(
          "notavailable"
        )
      ) {
        Severentfications[i].parentElement.parentElement.classList.toggle(
          "notavailable"
        );
      }
    }
