...
function autofillCredentials(username, password) {
    ...
    console.log([...document.getElementsByTagName('input')]);  // Use the spread operator to convert to array
    const allFields = [...document.getElementsByTagName('input')];  // Convert HTML collection to array
    ...
    allFields.forEach(field => {
        ...
        if (
            ...
            USERNAME_FIELDS.selectors.some(keyword => lowerCaseName.includes(keyword)) ||
            USERNAME_FIELDS.selectors.some(keyword => lowerCasePlaceholder.includes(keyword))
        ) {
        ...
        if (
            ...
            PASSWORD_FIELDS.selectors.some(keyword => lowerCaseName.includes(keyword)) ||
            PASSWORD_FIELDS.selectors.some(keyword => lowerCasePlaceholder.includes(keyword))
        ) {
...
chrome.runtime.sendMessage({ type: "getDummyCredentials" }, (credentials) => {
